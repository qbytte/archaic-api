            ...
            switch (req.method) {
                ...
                case 'POST':
                    utils.bodyParser(req, res, async () => {
                        db.run('INSERT INTO people (name, age) VALUES (?, ?)', [req.body.name, req.body.age], (err) => {
                            if (err) {
                                res.writeHead(500, { 'Content-Type': 'application/json' });
                                res.write(JSON.stringify({ error: err.message }));
                                res.end();
                ...
                case 'PUT':
                    utils.bodyParser(req, res, async () => {
                        db.run('UPDATE people SET name = ?, age = ? WHERE id = ?', [req.body.name, req.body.age, req.body.id], (err) => {
                            if (err) {
                                res.writeHead(500, { 'Content-Type': 'application/json' });
                                res.write(JSON.stringify({ error: err.message }));
                                res.end();
